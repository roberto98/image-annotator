...
function renderLandmarkPointLegacy(name, coords, color) {
    ...
    const { x, y } = viewport.imageToScreen(coords.x, coords.y);
...
function renderPolygonShapeLegacy(points, color, name) {
    ...
    const pointsStr = points.map(p => {
        const display = viewport.imageToScreen(p.x, p.y);
    ...
    const firstPoint = viewport.imageToScreen(points[0].x, points[0].y);
...
function renderLineShapeLegacy(figure, data, name, isInteractive) {
    const displayStart = viewport.imageToScreen(data.startX, data.startY);
    const displayEnd = viewport.imageToScreen(data.endX, data.endY);
...
function drawPolygonLineLegacy(p1, p2) {
    const display1 = viewport.imageToScreen(p1.x, p1.y);
    const display2 = viewport.imageToScreen(p2.x, p2.y);
